...
There are `r ncol(grade)-2` subjects in the final exam. The `r ncol(grade)-2` subjects are: `r colnames(grade)[3:10]`. `r nrow(grade)` students are collected in this project. When I was in high school, senior students and teachers told us grade of `r colnames(grade)[4]` and `r colnames(grade)[6]` could reflect the total grade. In other words, if I could get high grade in `r colnames(grade)[4]` and `r colnames(grade)[6]`, my total grades would not be low. Now, I will use the grades I collected to test this regular pattern.
...
```
From `t.test` above, p-value is `r t.test(math.p,phy.p)$p.value`, which is much smaller than critical value 0.05. Therefore, we can conclude that there is a significant difference between Math and Physics grade. In other words, students who do well in math, are not always good at physics.      
