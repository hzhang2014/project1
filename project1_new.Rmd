...
```
This is the project for the first half of the semester for course Microbiol 612.This project will analyze the final grades of first-year students from two different high schools. All the data are download from [wenku.baidu.com](http://wenku.baidu.com/view/09f6dc3752ea551811a68727.html).      

The project focuses on two high schools. There are `r summary(grade$School)["D"]` students in D shool that participated in this final exam, and `r summary(grade$School)["E"]` students in E school that participated in the exam.In sum,  `r nrow(grade)` students are collected in this project. There are `r ncol(grade)-2` subjects in the final exam. The `r ncol(grade)-2` subjects are: `r colnames(grade)[3:10]`. These two high schools are the Top 2 in the district. In this project, we will analyze the difference or similarity between them.
      
Both D school and E school are good, but are they really differ from each other? When I was in high school, there were some interesting phenomena, such as many of us could get a higher grade in Chinese than English, students who are good at Math also do well in Physics, and students with high grade in `r colnames(grade)[4]` and `r colnames(grade)[6]` also had higher total grades. Now, I will use the data I collected to test these interesting phenomena.
...
When solving physical problems, it is inevitable to use mathematical skills. ***Then, can we get a conclusion that higher Math grade contributes to higher Physics grade?*** We can test this by testing if there exist a positive linear correlation between Math grade and Physics grade.         
...
Though all the subjects contribute to the total grade, they might have different weight. ***Can we assume that Physics has greater impact on total grades?*** Here, we could test the weight of correlation for two subjects, Physics and History. These two subjects have the same maximum possible grade, 100.       
...
##Conclusion      
Students in E high school might get higher grades than students in D high school. But we cannot say E school is better than D school based on only one final exam. Students could do better in Chinese than English. This might be owing to Chinese is the native language for these students.  Math does help in Physics. Physics and total grade are more correlated than History and total grade.